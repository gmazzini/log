...
int qrzcom(MYSQL *con,char *call){
  ...
  if(key[6][0]!='\0'){
    sprintf(aux2,"replace into who (callsign,firstname,lastname,addr1,addr2,state,zip,country,grid,email,cqzone,ituzone,born,image,time,src) value ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%d,%d,%d,'%s','%lld','qrz.com')",call,key[0],key[1],key[2],key[3],key[4],key[5],key[6],key[7],key[8],atoi(key[9]),atoi(key[10]),atoi(key[11]),key[12],time(NULL));
...
int qrzru(MYSQL *con,char *call){
  ...
  if(key[6][0]!='\0'){
    sprintf(aux2,"replace into who (callsign,firstname,lastname,addr1,addr2,state,zip,country,grid,email,cqzone,ituzone,born,image,time,src) value ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%d,%d,%d,'%s','%lld','qrz.com')",call,key[0],key[1],key[2],key[3],key[4],key[5],key[6],key[7],"",atoi(key[8]),atoi(key[9]),atoi(key[10]+6),key[11],time(NULL));
    mysql_query(con,aux2);
    return 1;
  }
  return 0;
...
char *e2dtc(time_t e){
  static char out[20];
  strftime(out,sizeof(out),"%Y-%m-%d %H:%M:%S",gmtime(&e));
  return out;
}
