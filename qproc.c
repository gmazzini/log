...
int main(){
  ...
  for(entry=0;;entry++){
    ...
    if(row==NULL)break;
    sprintf(buf,"insert into wc (mycall,callsign,source) value ('%s','%s','me')",mycall,row[0]);
    mysql_query(con,buf);
    printf("%s\n",buf);
  }
  ...
  sprintf(buf,"select distinct callsign from log where mycall='%s' and callsign in (select callsign from wc where mycall='%s' and source!='me')",mycall,mycall);
  mysql_query(con,buf);
  res=mysql_store_result(con);
  for(entry=0;;entry++){
    row=mysql_fetch_row(res);
    if(row==NULL)break;
    sprintf(buf,"update wc set source='me' where mycall='%s' and callsign='%s'",mycall,row[0]);
    mysql_query(con,buf);
    printf("%s\n",buf);
  }
  mysql_free_result(res);
  ...
  for(entry=i=0;i<wcn;i++){
    sprintf(buf,"select count(*) from wc where mycall='%s' and callsign='%s'",mycall,wccall[i]);
    mysql_query(con,buf); res=mysql_store_result(con); row=mysql_fetch_row(res); c=atoi(row[0]); mysql_free_result(res);
    if(c==0){
      sprintf(buf,"insert into wc (mycall,callsign,source,sent,you) value ('%s','%s','oth',1,1)",mycall,wccall[i]);
      entry++;
    }
    else sprintf(buf,"update wc set sent=1,you=1 where mycall='%s' and callsign='%s'",mycall,wccall[i]);
    mysql_query(con,buf);
  }
  printf("--- %ld entries with %ld inserted\n\n",wcn,entry);

  printf(">> Set me on unset wc\n");
  sprintf(buf,"select callsign from wc where mycall='%s' and looked>0 and me=0 and you=1 and Ewc=1 order by Nwc desc",mycall);
  mysql_query(con,buf);
  res=mysql_store_result(con);
  for(updated=entry=0;;entry++){
    ...
    sprintf(buf,"update wc set me=1 where mycall='%s' and callsign='%s'",mycall,row[0]);
    mysql_query(con,buf);
    printf("%s\n",buf);
    updated++;
  }
  ...
  sprintf(buf,"select callsign from wc where mycall='%s' and sent=0 and qrzed=0 and source='me' order by rand()",mycall);
  mysql_query(con,buf);
  res=mysql_store_result(con);
  for(updated=entry=0;;){
    row=mysql_fetch_row(res);
    if(row==NULL)break;
    c=qrzcom(con,row[0]);
    sprintf(buf,"update wc set qrzed=%ld where mycall='%s' and callsign='%s'",time(NULL)/86400,mycall,row[0]);
    mysql_query(con,buf);
    printf("%s\n",buf);
    entry++;
    if(c==0)continue;
    ...
    if(strlen(youremail)>5 && c==0){
      ...
      sprintf(buf,"update wc set sent=1 where mycall='%s' and callsign='%s'",mycall,row[0]);
      mysql_query(con,buf);
      printf("%s\n",buf);
  ...
  sprintf(buf,"select callsign,Nwc from wc where mycall='%s' and sent=0 and qrzed=0 and source='oth' and me=0 and you=0 and Ewc=1 order by Nwc desc",mycall);
  mysql_query(con,buf);
  res=mysql_store_result(con);
  for(updated=entry=0;;){
    sleep(3+rand()%5);
    ...
    if(setqrz(row[0])==0)continue;
    entry++;
    sprintf(buf,"update wc set me=1,qrzed=%ld where mycall='%s' and callsign='%s'",time(NULL)/86400,mycall,row[0]);
    mysql_query(con,buf);
    printf("%s\n",buf);
    sleep(3+rand()%5);
    c=qrzcom(con,row[0]);
  ...
  sprintf(buf,"select callsign from wc where mycall='%s' and sent=1 and qrzed=0 order by rand()",mycall);
  mysql_query(con,buf);
  res=mysql_store_result(con);
  for(entry=0;;){
    row=mysql_fetch_row(res);
