    ...
    for(;;){
      ...
      for(;;){
        ...
        *qq='\0';
        ...
        sprintf(buf,"select count(*) from log where mycall='%s' and callsign='%s'",mycall,p4);
        
        mysql_query(con,buf); res=mysql_store_result(con); row=mysql_fetch_row(res);
        if(row[0]==NULL){l3=0; l4=0; strcpy(aux1,"   ");} else{l3=atol(row[0]); l4=atol(row[1]); strcpy(aux1,myts(atol(row[2])));}
        mysql_free_result(res);
