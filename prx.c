...
int main(void) {
  ...
  for(;;){
    ...
    sprintf(aux3,"('%s','%s','%s','%s','%s',%ld,%ld,'%s','%s','%s','%s','%s',%d,%lld,%lld)",adif[16],adif[0],aux1,aux2,adif[5],(long)(atof(adif[1])*1000000.0),(long)(atof(adif[2])*1000000.0),adif[3],adif[4],(adif[8][0]=='\0')?adif[9]:adif[8],(adif[10][0]=='\0')?adif[11]:adif[10],adif[12],atoi(mycty[2]),dt2e(adif[13],adif[6]),dt2e(adif[14],adif[7]););
    fp=fopen(LOG,"a");
    if(fp!=NULL){fprintf(fp,"%s\n",aux3); fclose(fp);}
    sprintf(buf,"insert ignore into log (mycall,callsign,start,end,mode,freqtx,freqrx,signaltx,signalrx,contesttx,contestrx,contest,dxcc,open,close) value %s",aux3);
    mysql_query(con,buf);
    mysql_close(con);
  }
}
