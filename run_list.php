...
for(;;){
  ...
  printf("<button type=\"button\" onclick=\"mydel('%s','%s','%s')\">D</button> ",$mycall,$row["start"],$row["callsign"]);
...
function mydel(mycall,start,callsign){
  ...
  xmlhttp.open("GET","act_booking.php?mycall="+mycall+"&act=D|"+start+"|"+callsign,true);
  xmlhttp.send();
}
</script>
